#!/usr/bin/env bash
# Integration tests for go-s3-server using curl.
set -euo pipefail
...
curl -sf -X PUT --data-binary @/tmp/test-upload \
  "$ENDPOINT_NORMAL/$BUCKET/api/v1test000000000001" > /dev/null
curl -sf -o /tmp/test-download \
...
# GetObject 404
HTTP_CODE=$(curl -s -o /tmp/test-404-body -w "%{http_code}" \
...
# ListObjectsV2
for i in 0 1 2; do
  curl -sf -X PUT --data-binary "data" \
...
curl -sf -X PUT --data-binary "first" \
  "$ENDPOINT_NORMAL/$BUCKET/overwrite/v1test000000000002" > /dev/null
curl -sf -X PUT --data-binary "second" \
  "$ENDPOINT_NORMAL/$BUCKET/overwrite/v1test000000000002" > /dev/null
OVERWRITE_BODY=$(curl -sf \
...
curl -sf -X PUT --data-binary "sharddata" \
...
# First PUT succeeds
curl -sf -X PUT --data-binary "first write" \
...
curl -sf -X PUT --data-binary "idempotent content" \
...
curl -sf -X PUT --data-binary "original" \
...
curl -sf -X PUT --data-binary "wosharddata" \
