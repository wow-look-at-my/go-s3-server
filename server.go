...
type Server struct {
	...
	// server is at capacity, so an excess request is shed with 503 and
	// Retry-After rather than queued until memory is gone, which a fronting
	// proxy reports as a 502. A request whose remaining work is only a body
